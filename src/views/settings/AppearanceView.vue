    ...
    <div class="flex flex-col h-screen bg-gray-50">
        <header class="bg-white border-b border-gray-200 px-4 py-4">
            <div class="flex items-center justify-between">
                ...
                <h1 class="text-xl font-bold text-gray-800">{{ $t('settings.appearance.title') }}</h1>
                <div class="w-6"></div>
            </div>
        </header>
        <main class="flex-1 overflow-y-auto px-4 py-5">
            <!-- Language Selector -->
            <div class="mb-6">
                <h3 class="font-bold text-gray-800 mb-3">{{ $t('settings.appearance.language') }}</h3>
                <div class="space-y-3">
                    <button @click="changeLanguage('de')" :class="[
                        'w-full flex items-center justify-between p-4 rounded-xl border-2 transition',
                        currentLocale === 'de' ? 'border-primary bg-blue-50' : 'border-gray-200 bg-white'
                    ]">
                        <div class="flex items-center gap-3">
                            <span class="text-2xl">🇩🇪</span>
                            <span class="font-semibold text-gray-800">Deutsch</span>
                        </div>
                        <svg v-if="currentLocale === 'de'" class="w-6 h-6 text-primary" fill="currentColor"
                            viewBox="0 0 20 20">
                    ...
                    <button @click="changeLanguage('en')" :class="[
                        'w-full flex items-center justify-between p-4 rounded-xl border-2 transition',
                        currentLocale === 'en' ? 'border-primary bg-blue-50' : 'border-gray-200 bg-white'
                    ]">
                        <div class="flex items-center gap-3">
                            <span class="text-2xl">🇬🇧</span>
                            <span class="font-semibold text-gray-800">English</span>
                        </div>
                        <svg v-if="currentLocale === 'en'" class="w-6 h-6 text-primary" fill="currentColor"
                            viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                clip-rule="evenodd" />
            ...
            <div class="mb-6">
                <div class="bg-yellow-100 rounded-xl p-3 flex items-center gap-2 mb-4">
                    ...
                    <div>
                        <div class="text-xs text-yellow-900 font-semibold">{{ $t('profile.coins') }}</div>
                        <div class="text-lg text-yellow-900 font-bold">{{ userStore.profile.coins }}</div>
                    </div>
                </div>
                <h3 class="font-bold text-gray-800 mb-3">{{ $t('settings.appearance.theme') }}</h3>
                <div class="space-y-3">
                    <button v-for="theme in themesStore.themes" :key="theme.id" @click="selectTheme(theme)" :class="[
                        ...
                        <svg v-if="themesStore.currentTheme === theme.id && theme.unlocked" class="w-6 h-6 text-primary"
